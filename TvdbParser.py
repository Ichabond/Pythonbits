...
class TVDB(object):
    ...
    def summary(self):
        if isinstance(self.episode, tvdb_api.Episode) and not self.season:
            return {'title': self.episode['episodename'], 'director': self.episode['director'],
                    'aired': self.episode['firstaired'],
                    'writer': self.episode['writer'], 'rating': self.episode['rating'],
                    'summary': self.episode['overview'],
                    'language': self.episode['language'], 'genre': self.tvdb[int(self.episode['seriesid'])]['genre'],
                    'url': "http://thetvdb.com/?tab=episode&seriesid=" + self.episode['seriesid'] + "&seasonid=" +
            ...
            for (counter, episode) in enumerate(self.season):
                summary['url'] = "http://thetvdb.com/?tab=season&seriesid=" + self.season[episode][
                    'seriesid'] + "&seasonid=" + self.season[episode]['seasonid']
        ...
        if isinstance(self.show, tvdb_api.Show):
            return {'series': self.show['seriesname'], 'seasons': len(self.show), 'network': self.show['network'],
                    'rating': self.show['rating'], 'contentrating': self.show['contentrating'],
